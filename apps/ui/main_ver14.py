...
with st.sidebar:
    ...
    # ✅ Checkbox to toggle endpoint testing
    show_testing = st.checkbox("Show Direct Endpoint Testing", value=False)
...
with chat_container:
    for msg in st.session_state.messages:
        with st.chat_message(msg["role"]):
            # Defensive rendering: show placeholder if content is empty
            content = msg.get("content", "⚠️ Empty message")
            st.markdown(
                f"{content}\n\n"
                f"<span style='font-size:0.75em;opacity:0.6'>{msg.get('time','')}</span>",
