...
#     # --- Structured queries with order ID ---
...
#         "refund policy", "how do i get a refund", "how refunds work", "refund process",
#         "return policy", "how do i return", "return process", "exchange policy"
#     ]):
#         return "faq"

#     # --- Tracking without order ID ---
#     if "track my order" in q or "where is my order" in q:
#         return "faq"
...
def classify_intent(user_query: str) -> dict:
    ...
    if "refund" in q:
        return {
            ...
            "needs_order_id": not has_order_id
        }

    if "return" in q:
        return {
            "intent": "return_policy" if not has_order_id else "return_status",
            "needs_order_id": not has_order_id
        }
...
# FAQ_PATH = os.path.join(BASE_DIR, "data", "faq_docs.json")
...
def chat(req: ChatRequest):
    ...
    if needs_order_id:
        return {
            "message": "❗ Please provide a valid order number (e.g., 12345)."
        }
